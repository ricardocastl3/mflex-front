...
import useFootballTeams from "@/hooks/api/football/useFootballTeams";
...
export default function ContentCategory({
  ...
  const { allFootballTeams, isLoadingAllFootballTeams, handleSeachByName } =
    useFootballTeams();
  const { handleSelectedFootballAPITeam } = useFootballProvider();
      ...
      <div className="flex items-center justify-between pb-2 mb-2 border-b  border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-normal font-bold  dark:text-slate-100 ">
            <CTranslateTo pt="Equipas" eng="Teams" />
          </h3>
        </div>
        ...
        {isLoadingAllFootballTeams && (
          <div className="flex justify-center items-center pt-8 dark:text-white">
            <AuSoftUI.Component.isFormSubmitting
              isSubmitting={isLoadingAllFootballTeams}
            />
          </div>
        )}
        {!isLoadingAllFootballTeams && allFootballTeams.length > 0 && (
          <>
            {allFootballTeams.map((category, i) => {
              return (
                <AuSoftUI.UI.Button
                  type="button"
                  onClick={() => {
                    handleSelectedFootballAPITeam(category);
                    {
                      callback && callback();
                    }
                  }}
        ...
        {!isLoadingAllFootballTeams && allFootballTeams.length <= 0 && (
          <div className="flex items-center flex-col gap-2 mt-4">
            <h4 className="text-center text-normal dark:text-white font-bold">
              <CTranslateTo eng="Team not found" pt="Equipa não encontrada" />
            </h4>
          </div>
        )}
      </div>
