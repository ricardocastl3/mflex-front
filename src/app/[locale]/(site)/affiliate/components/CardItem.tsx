...
export default function CardItem({ service }: { service: IServiceAffiliate }) {
  return (
    <BaseBox className="cursor-pointer justify-between h-full p-4 flex flex-col gap-4 md:hover:scale-[1.03] hover:scale-100 transition-all border-l hover:border-l-yellow-500 dark:hover:border-l-yellow-800">
      ...
      {service.action != "" && (
        <div className="flex justify-center gap-4">
          <Link
            target="_blank"
            href={`/${langByCookies}/${service.action}`}
            className="w-full"
          >
            <AuSoftUI.UI.Button
              variant={"primary"}
