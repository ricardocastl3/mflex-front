...
export default function SubsCard({
  ...
  const isExpired = currentArtistSubscription
    ? currentArtistSubscription.subscription.is_expired
    : true;

  const samePlan =
    currentArtistSubscription &&
    currentArtistSubscription.subscription.plan?.id == plan.id
      ? true
      : false;

  const buttonDisabled =
    (!isExpired && samePlan) ||
    (currentArtistSubscription && userLogged?.is_trialed && plan.is_trial);
