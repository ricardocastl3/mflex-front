...
export default function CRTCommentDeletionGlobalModal() {
  const { handleFetchResource } = useResourceProvider();
  const { selectedDelFHComment, handleSelectDelFHComment } =
  ...
  async function handleDelete() {
    try {
      ...
      const route = selectedDelFHComment.type == "comment" ? "" : "/res";
      await internalApi.delete("/users/comments" + route, {
        data: {
          id: selectedDelFHComment.id,
        },
      });
      setIsSubmitting(false);
      handleFetchResource(true);
      handleSelectDelFHComment({ id: "", type: "" });
      callbackClose();
    } catch (err) {
      setIsSubmitting(false);
